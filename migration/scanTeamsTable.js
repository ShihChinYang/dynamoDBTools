...
var TABLE_NAME = "com.bsafes.teamsV2";
var INDEX_NAME = "teamsV2TimeStamp"; // GSI: partition key keyVersion, sort key createdTime
...
if (keyVersionArg === undefined) {
  ...
  console.error("Usage: node scanTeamsTable.js <keyVersion> [timeStamp]");
  process.exit(1);
}
...
// Optional: only consider teams created after this createdTime.
...
var CHECKPOINT_FILE = path.join(__dirname, "scanTeamsTable.keyVersion-" + KEY_VERSION_VALUE + ".checkpoint.json");
...
// Read the full team item from the base table using the id from the GSI.
...
// Check whether this id is already in the 'team' index.
...
// Index the full team item to the 'team' OpenSearch index (same shape scanTeamsTable.js used to build).
function indexItem(team) {
  var indexBody = {
    name: team.name,
    IV: team.IV,
    createdBy: team.createdBy,
    createdTime: team.createdTime,
    searchKeyEnvelope: team.searchKeyEnvelope,
    searchKeyIV: team.searchKeyIV
  };
  ...
  return client.index({
    ...
    console.log("Could not index team: <team.id>", teamId);
    throw error;
  });
}
