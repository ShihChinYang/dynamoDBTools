...
var INDEX_NAME = "teamMembersV2TimeStamp"; // GSI: partition key keyVersion, sort key joiningTime
...
if (keyVersionArg === undefined) {
  ...
  console.error("Usage: node scanTeamMembersTable.js <keyVersion> [joiningTime]");
  process.exit(1);
}
...
// Optional: only consider team members who joined after this joiningTime.
...
if (timeStampArg !== undefined) {
  ...
  KEY_CONDITION += " AND joiningTime > :ts";
  EXPRESSION_VALUES[":ts"] = TIMESTAMP_VALUE;
  console.log("Querying for keyVersion:", KEY_VERSION_VALUE, "with joiningTime >", TIMESTAMP_VALUE);
} else {
...
var params = {
  ...
  ScanIndexForward: true // ascending by joiningTime
};
...
var CHECKPOINT_FILE = path.join(__dirname, "scanTeamMembersTable.keyVersion-" + KEY_VERSION_VALUE + ".checkpoint.json");
...
// Read the full teamMember item from the base table using the teamId+memberId from the GSI.
function getFullItem(teamId, memberId) {
...
// Check whether this teamId+memberId is already in the 'teammember' index.
...
// Index the full teamMember item to the 'teammember' OpenSearch index (same shape scanTeamMembersTable.js used to build).
function indexItem(teamMember) {
  var teamMemberId = teamMember.teamId + '-' + teamMember.memberId;

  if (teamMember.status === "deleted") {
    console.log("This team member was deleted, skipping:", teamMemberId);
  ...
  } else if (teamMember.joiningTime) {
    position = teamMember.joiningTime;
  } else {
    position = 0;
  }
  ...
  var indexBody = {
    ...
    teamKeyEnvelope: teamMember.teamKeyEnvelope,
    position: position
  };

  if (teamMember.encryptedTeamName) {
    indexBody.encryptedTeamName = teamMember.encryptedTeamName;
  }

  if (teamMember.cachedTeamName) {
    indexBody.cachedTeamName = teamMember.cachedTeamName;
  }
  ...
  return client.index({
    ...
    console.log("Could not index teamMember: <teammember.id>", teamMemberId);
    throw error;
  });
}
...
async function processItem(gsiItem) {
  var id = gsiItem.teamId + '-' + gsiItem.memberId;

  for (var attempt = 1; ; attempt++) {
    try {
      var exists = await alreadyIndexed(id);
      if (exists) {
        console.log("Already indexed, skipping:", id);
        return;
      }

      var fullItem = await getFullItem(gsiItem.teamId, gsiItem.memberId);
      if (!fullItem) {
        console.error("Base table has no item for teamId:", gsiItem.teamId, "memberId:", gsiItem.memberId, "- skipping");
        number++;
        return;
      }
      ...
      var label = "Processing " + id + " failed";
...
async function queryIndex() {
  if (timeStampArg !== undefined) {
    console.log("joiningTime argument provided - ignoring any saved checkpoint, starting fresh from that point.");
  } else {
  ...
  while (true) {
    ...
    for (var i = 0; i < items.length; i++) {
      itemIndex++;
      console.log("[" + itemIndex + "] Processing teamId:", items[i].teamId, "memberId:", items[i].memberId);
      await processItem(items[i]);
    }

    if (items.length > 0) {
      // Remember our position: the GSI key (keyVersion, joiningTime) plus the
      // base table key (teamId, memberId) that DynamoDB requires for a GSI cursor.
      var lastItem = items[items.length - 1];
      params.ExclusiveStartKey = {
        keyVersion: KEY_VERSION_VALUE,
        joiningTime: lastItem.joiningTime,
        teamId: lastItem.teamId,
        memberId: lastItem.memberId
      };
    }
