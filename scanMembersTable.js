...
var tableName = "com.bsafes.membersV2";
if(useOpenBSafes) {
...
var params = {
    TableName : tableName,
    ProjectionExpression: "#id, #email , displayName, passwordHash, currentKeyVersion, keyHashVersions",
...
function indexItem(member, done) {
	console.log(member);
  ...
  // The OpenSearch client is promise-based (no callback form) and mapping
  // types were removed, so there is no `type` parameter anymore.
  client.index({
    index: 'member',
    id: memberId,
    body: indexBody
    ...
    console.log("number:", number);
		...
		number ++;
    console.log("number:", number);
		console.log("Could not index member: <member.id>", member.id);
		done(error);
  });
}
